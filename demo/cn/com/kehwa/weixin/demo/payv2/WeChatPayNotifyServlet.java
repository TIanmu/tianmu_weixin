...
public class WeChatPayNotifyServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			boolean temp = PayV2.verifySign(timestamp, noncestr, openid, issubscribe, appsignature);
			if (!temp) {
		...
		// 发送客服消息
		try {
			WeChat.message.sendText(orderId, "您的订单号" + orderId + "已经支付成功！");
			//发货通知！推荐支付成功后就发送此通知...
			PayV2.delivernotify(openid, transId, transId);
		} catch (Exception e) {
			e.printStackTrace();
		}
